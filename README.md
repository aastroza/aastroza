...
- **[November 2025]** 🗣️ Released [shiaaa.cl](https://shiaaa.cl/), a community-driven crowdsourcing platform to collect high-quality Chilean Spanish speech data. Current multilingual ASR models claim strong Spanish performance but fail badly on Chilean audio, so building an accurate model requires new data. [This platform](https://github.com/idsudd/shiaaa) lets anyone contribute transcripts that will be fully open-sourced along with the models trained on them.
- **[October 2025]** 🎧 Launched the [Open ASR Leaderboard for Chilean Spanish](https://huggingface.co/spaces/idsudd/open_asr_leaderboard_cl), benchmarking state-of-the-art speech recognition models on real Chilean speech from Common Voice, Google’s Chilean Spanish dataset, and Datarisas (Festival de Viña del Mar comedy segments). Results show a clear gap: models perform well on standardized Spanish but struggle with natural, culturally grounded Chilean speech. I’m now training an open ASR model optimized specifically for Chilean Spanish and looking for collaborators (data, annotation, compute, or funding).
